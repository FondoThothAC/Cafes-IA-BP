...
from modules import BusinessStandards, DeltaLogicEngine
...
def list_all_projects() -> Dict[str, Any]:
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("""
            SELECT id_proyecto, a1_nombre_negocio, a2_nombre_emprendedor, 
                   estatus_proyecto, fecha_creacion
            ...
            "projects": [
                {
                    ...
                    "created": str(p["fecha_creacion"]) if p["fecha_creacion"] else None
                }
                for p in projects
            ]
        }
...
@mcp.tool()
def list_available_standards(project_id: int) -> Dict[str, Any]:
    """
    Lista los modelos de negocio estándar disponibles para generar.
    
    Args:
        project_id: ID del proyecto (para inicializar el contexto)
        
    Returns:
        Lista de modelos soportados (Canvas, Lean, SWOT, etc.)
    """
    project = fetch_project(project_id)
    if not project:
        return {"error": f"Proyecto {project_id} no encontrado"}
    ...
    return {
        "project_id": project_id,
        "available_models": standards.get_available_models()
    }


@mcp.tool()
def get_standard_model(project_id: int, model_type: str) -> Dict[str, Any]:
    """
    Genera un modelo de negocio estándar con los datos del proyecto.
    
    Use 'list_available_standards' para ver las opciones válidas.
    
    Args:
        project_id: ID del proyecto source
        model_type: Tipo de modelo (ej: 'business_model_canvas', 'swot_analysis')
        
    Returns:
        Diccionario con la estructura del modelo llenada
    """
    project = fetch_project(project_id)
    if not project:
        return {"error": f"Proyecto {project_id} no encontrado"}
    ...
    try:
        standards = BusinessStandards(project)
        model_data = standards.generate_model(model_type)
        return {
            "project_id": project_id,
        ...
        return {"error": f"Internal error generating model: {str(e)}"}


@mcp.tool()
def get_delta_diagnosis(project_id: int) -> Dict[str, Any]:
    """
    Obtiene el diagnóstico estratégico del Modelo Delta (Lógica Determinista).
    
    Analiza la distribución de productos y competidores para recomendar
    una posición estratégica (Best Product, Total Customer Solution, System Lock-In).
    
    Args:
        project_id: ID del proyecto
        
    Returns:
        Diccionario con la posición recomendada, score y razonamiento.
    """
    project = fetch_project(project_id)
    if not project:
        return {"error": f"Proyecto {project_id} no encontrado"}
    ...
    try:
        # We need to ensure JSON fields are parsed or available as strings for the engine
        # The fetch_project returns dictionary with strings for JSON columns usually
        delta_engine = DeltaLogicEngine(project)
        analysis = delta_engine.analyze_position()
        ...
        return {"error": f"Error calculating Delta position: {str(e)}"}
